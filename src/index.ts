...
export class YePanel extends plugin {
  constructor () {
    super({
      ...
      rule: [
        ...
        {
          reg: /^#?(小叶|YePanel|YP)(面板)?(代理|proxy)?安装web$/i,
          fnc: 'installWeb'
        }
      ]
    })
  ...
  async installWeb (e: any) {
    ...
    const isProxy = ['代理', 'proxy'].some(i => e.msg.includes(i))
    const cmd = `git clone --depth=1 -b gh-pages ${isProxy ? 'https://github.moeyy.xyz/' : ''}https://github.com/XasYer/YePanel.git ./plugins/YePanel-Web/`
    try {
      execSync(cmd)
    } catch (error) {
      e.reply(`安装web服务失败: ${(error as Error).message}, 请检查网络或${isProxy ? '手动克隆仓库到plugins目录下' : '发送#小叶面板代理安装web 使用proxy加速进行安装'}, 安装命令: \n${cmd}`)
      run = false
      return
    }
