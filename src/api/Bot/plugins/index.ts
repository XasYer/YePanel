import { getPlugins } from '@/api/welcome'
...
// From Guoba-Plugin
function parseReadmeLink (text: string, baseUrl: string) {
  const linkReg = /\[.*]\((.*)\)/g
...
const output: {[key: string]: {error?: Error | null, stdout?: string, stderr?: string, running?: boolean}} = {}
...
export default [
  {
    ...
    handler: async () => {
      ...
      const fileName: {
        name: 'README.md' | 'Function-Plugin.md' | 'Game-Plugin.md' | 'WordGame-Plugin.md',
        id: 'main' | 'function' | 'game' | 'wordgame'
      }[] = [
        {
          name: 'README.md',
          id: 'main'
  ...
  },
  {
    ...
    method: 'GET',
    handler: async ({ query }) => {
      const { link: pluginLink } = query as { link: string }
  ...
  },
  {
    ...
    method: 'POST',
    handler: async ({ body }) => {
      const { link, name, branch } = body as { link: string, name?: string, branch?: string }
      const pluginName = name || link.split('/').pop()?.replace(/\.git$/, '') as string
