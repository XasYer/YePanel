...
async function getDauChartData (uin: string) {
...
async function getWeekChartData (uin: string) {
  ...
  const receiveMsgData: number[] = []
  const sendMsgData: number[] = []
  data.coldata[1].forEach((v: {count: number, time: string}, i: number) => {
    if (i % 2 === 0) {
  ...
  data.linedata[0].forEach((v: { time: string, linecount: number }, i: number) => {
    if (i % 2 === 0) {
...
async function getcallStat (uin: string) {
  const dau = Bot[uin].dau
  const callStat = _.entries(dau.call_stats).sort((a, b) => b[1] - a[1])
  const data = await dau.callStat(callStat, true)
  return data.group.map((i: { num: number, percent: string, color: string, name: string }) => ({
    num: i.num,
