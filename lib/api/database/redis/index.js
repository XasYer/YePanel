...
const clients = {};
const getRedisClient = async (db, host, port) => {
    if (!host) {
        host = cfg.redis.host;
        port = cfg.redis.port;
    }
...
export async function getRedisKeys(sep = ':', db, host, port, lazy = false) {
...
export default [
    ...
    },
    {
        ...
        method: 'get',
        handler: async ({ query }) => {
            const { sep, db, lazy, host, port } = query;
    ...
    },
    {
        ...
        method: 'post',
        handler: async ({ body }) => {
            const { key: oldKey, value, db, expire, newKey, host, port } = body;
            const redis = await getRedisClient(db, host, port);
            if (!redis) {
                return {
                    success: false,
